...
$$
The experiment does not hand us a free $z$: the sweep puts $z$ on the unit circle, $z=e^{i\delta\tau}$. Restoring the phase $z^{-d/2}$ split off the waits,
$$
...
The measurement squares it, and $|\hat P|^{2}=\hat P\hat P^{*}$ is a product of two such series. By the one identity of Section 2 a product of series is a convolution of lists, so
$$
...
Take $d=1$, two taps. Then
$$
|\hat P|^{2}=|p_0|^{2}+|p_1|^{2}+2\,\mathrm{Re}\!\left(p_1p_0^{*}\,e^{i\delta\tau}\right) ,
$$
a constant plus one cosine. Two $\pi/2$ pulses of opposite sign around one wait give $p_0=p_1=\tfrac12$ and $\tfrac12\left(1+\cos\delta\tau\right)$, the Ramsey fringe of Section 1.
