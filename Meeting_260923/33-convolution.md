...
$$
Multiplying by $\Pi_0$ keeps the first column of $G_k$ and deletes the second, and $\Pi_1$ does the reverse. For example, for a pulse about $x$ of area $\beta_k$, writing $c=\cos\frac{\beta_k}{2}$ and $s=\sin\frac{\beta_k}{2}$,
$$
...
$$
So each step of the sequence is a **two-tap kernel whose taps are matrices**. The tap at $z^{0}$ sends the $\ket{0}$ amplitude through the pulse at the same power of $z$, and the tap at $z^{1}$ sends the $\ket{1}$ amplitude through the pulse one power of $z$ later.
